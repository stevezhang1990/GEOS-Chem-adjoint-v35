...
!
      ...
      INTEGER                         :: NR, NS, ST,TMP2
      INTEGER, DIMENSION(2)           :: DIMIDS

      CHARACTER(LEN=255)              :: MY_DIR
      ...
      READ_FILENAME = TRIM('teso3irk.YYYYMMDD.nc')
     ...
     &                TRIM( READ_FILENAME )
      ...
      CALL CHECK( NF90_INQ_VARID( FID, "ozone_vmr",       O3_ID ), 102 )
...
!     CALL CHECK( NF90_INQ_VARID( FID, "constraintvector",AP_ID ), 107 )
      ...
      ! IRK
      CALL CHECK( NF90_INQ_VARID( FID, "O3IRK",       RF_ID ), 111 )
      CALL CHECK( NF90_INQ_VARID( FID, "Tropopause_Pressure",
     &            TP_ID ), 112 )
...
!      CALL CHECK( NF90_INQ_VARID( FID, "tropopause",      TP_ID ), 112 )
     ...
     &      TES(N)%TPAUSE,       START0, COUNT0 ), 304 )
      ...
      DO N = 1, NTES
         ...
         ! Update starting index
         START1(1) = N
         ST = NF90_INQUIRE_VARIABLE(FID, O3_ID, TMP,TMP2,TMP2,DIMIDS)
         CALL CHECK(NF90_INQUIRE_DIMENSION(FID, DIMIDS(1), TMP, NR),305)
         CALL CHECK(NF90_INQUIRE_DIMENSION(FID, DIMIDS(2), TMP, NS),306)
     ...
     &      TES(N)%O3(1:MAXLEV), START1, COUNT1 ), 401 )
      ...
      DO N = 1, NTES
         ...
         ! READ O3 column, O3
         CALL CHECK( NF90_GET_VAR  ( FID, O3_ID,
     &      TES(N)%O3(1:J),      START1, COUNT1 ), 402 )


         ! READ pressure levels, PRES
         CALL CHECK( NF90_GET_VAR  ( FID, PS_ID,
     &      TES(N)%PRES(1:J),    START1, COUNT1 ), 403 )
...
!      READ_FILENAME = TRIM( 'tes_aura_nadir_irk_YYYYMMDD_O3_v4.nc' )
!      CALL EXPAND_DATE( READ_FILENAME, YYYYMMDD, 9999 )
!      READ_FILENAME = TRIM( MY_DIR ) // TRIM( READ_FILENAME )
!      WRITE(6,*) '    - READ_TES_O3_OBS: reading file: ', READ_FILENAME
!
!      ! Open file and assign file id (FID)
!      CALL CHECK( NF90_OPEN( READ_FILENAME, NF90_NOWRITE, FID ), 0 )
...
!      ENDDO  ! N
      ...
      ELSE
        ...
        PRINT*, 'No IRK file on this date'
        RETURN
...
!     &       IOSTAT=IOS, FORM='FORMATTED',    ACCESS='SEQUENTIAL' )
     ...
     &       IOSTAT=IOS, FORM='FORMATTED',    ACCESS='SEQUENTIAL' )
      ...
      IF ( GET_NHMS() == 233000 - GET_TS_CHEM() * 100 ) THEN
...
!!$OMP+PRIVATE( DIFF_ADJ                                )
      DO NT  = NTSTART, NTSTOP, -1
         ...
         ! Check IRK for NaN
         ! A NaN value is by definition not equal to anything even itself
         IF ( .NOT. TES(NT)%IRK(10) .GE. 0d0 .AND.
     &        .NOT. TES(NT)%IRK(10) .LE. 0d0 ) THEN
           print*, ' - found NaN in IRK, continue... '
           CONTINUE
         ENDIF
