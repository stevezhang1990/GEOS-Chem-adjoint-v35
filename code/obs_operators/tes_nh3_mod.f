!$Id: tes_nh3_mod.f,v 1.7 2011/02/23 00:08:48 daven Exp $(?)
      ...
      TYPE TES_NH3_OBS
         ...
         REAL*8                                  :: LAT
         REAL*8                                  :: LON
         REAL*8,  DIMENSION(LLTES)               :: NH3
         REAL*8,  DIMENSION(LLTES,LLTES)         :: AVG_KERNEL
         REAL*8,  DIMENSION(LLTES)               :: PRES
         REAL*8,  DIMENSION(LLTES,LLTES)         :: OER_INV
         REAL*8,  DIMENSION(LLTES)               :: PRIOR
         REAL*8,  DIMENSION(LLTES,LLTES)         :: AVK_VMR
         REAL*8,  DIMENSION(LLTES,LLTES)         :: OEI_VMR
         CHARACTER(LEN=255)                      :: FILENAME
         REAL*8                                  :: BLVMR
         REAL*8,  DIMENSION(LLTES)               :: BLVMR_WGT
      ENDTYPE TES_NH3_OBS
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) FILENAME   (REAL*8) : TES observation filename to read
!
!  Arguments as Output:
!  ============================================================================
!  (1 ) NH3        (REAL*8) : Retrieval NH3                        [ppmv]
!  (2 ) AVG_KERNEL (REAL*8) : Retrieval averaging kernel           [lnvmr/lnvmr]
!  (3 ) PRES       (REAL*8) : Retrieval pressure levels            [mbar]
!  (4 ) OER_INV    (REAL*8) : TES retrieval error matrix inverse   [((lnvmr))^-2]
!  (5 ) LLNT       (REAL*8) : TES retrieval number of levels
!  (6 ) PRIOR      (REAL*8) : TES retrieval prior                  [ppmv]
!  (7 ) QFLAG     (INTEGER) : TES retrieval quality flag
!  (8 ) DFLAG     (INTEGER) : TES retrieval diagnostic flag
!  (9 ) LAT        (REAL*8) : TES retrieval latitude
!  (10) LON        (REAL*8) : TES retrieval longitude
!  (11) BLVMR      (REAL*8) : TES retrieval BLVMR                  [ppmv]
!  (12) BLVMR_WGT  (REAL*8) : TES retrieval BLVMR mapping          [lnblvr/lnppmv]
!
!  NOTES:
...
#     include "CMN_SIZE"
      ...
      ! Arguments
      REAL*8,             INTENT(OUT) :: NH3(LLTES)
      REAL*8,             INTENT(OUT) :: AVG_KERNEL(LLTES,LLTES)
      !REAL*8              INTENT(OUT) :: AVK_VMR(LLTES,LLTES)
      REAL*8,             INTENT(OUT) :: PRES(LLTES)
      !REAL*8,             INTENT(OUT) :: OEI_VMR(LLTES,LLTES)
      REAL*8,             INTENT(OUT) :: OER_INV(LLTES,LLTES)
      REAL*8,             INTENT(OUT) :: PRIOR(LLTES)
      INTEGER,            INTENT(OUT) :: LLNT
      INTEGER,            INTENT(OUT) :: QFLAG
      INTEGER,            INTENT(OUT) :: DFLAG
      REAL*8,             INTENT(OUT) :: LAT
      REAL*8,             INTENT(OUT) :: LON
      REAL*8,             INTENT(OUT) :: BLVMR
      REAL*8,             INTENT(OUT) :: BLVMR_WGT(LLTES)
...
!  ============================================================================
!  (1 ) FILENAME   (REAL*8) : TES observation filename to read
!  (2 ) BLVMR_GC   (REAL*8) : Boundary layer volume mixing ration in GC [ppmv]
!  (3 ) NH3_GC     (REAL*8) : GEOS-Chem NH3 concentrations on TES grid  [ppmv]
!  (4 ) NH3_HAT    (REAL*8) : GEOS-Chem NH3 after applying TES obs op   [ppmv]
...
#     include "CMN_SIZE"
      ...
      ! Arguments
      CHARACTER(LEN=255), INTENT(IN)  :: FILENAME
      REAL*8,             INTENT(IN)  :: BLVMR_GC
      REAL*8,             INTENT(IN)  :: NH3_GC(LLTES)
      REAL*8,             INTENT(IN)  :: NH3_HAT(LLTES)
...
#     include      "CMN_SIZE"      ! Size params
      ...
      REAL*8                      :: GC_NH3_NATIVE(LLPAR)
      REAL*8                      :: GC_NH3(LLTES)
      REAL*8                      :: GC_PSURF
      REAL*8                      :: MAP(LLPAR,LLTES)
      REAL*8                      :: NH3_HAT(LLTES)
      REAL*8                      :: NH3_PERT(LLTES)
      REAL*8                      :: FORCE(LLTES)
      REAL*8                      :: DIFF(LLTES)
      REAL*8                      :: NEW_COST(MAXTES)
      REAL*8                      :: OLD_COST

      REAL*8                      :: GC_NH3_NATIVE_DBL(LLPAR)
      REAL*8                      :: GC_NH3_DBL(LLTES)
      REAL*8                      :: NH3_HAT_DBL(LLTES)
      REAL*8                      :: NH3_PERT_DBL(LLTES)

      REAL*8                      :: ADJ_GC_NH3_NATIVE(LLPAR)
      REAL*8                      :: ADJ_NH3_HAT(LLTES)
      REAL*8                      :: ADJ_NH3_PERT(LLTES)
      REAL*8                      :: ADJ_GC_NH3(LLTES)
      REAL*8                      :: ADJ_DIFF(LLTES)
      REAL*8                      :: BLVMR_GC
      REAL*8                      :: BLVMR_TC
      REAL*8                      :: TMP1
...
!  ============================================================================
!  (1 ) NH3_SAVE (REAL*8) : NH3 profiles                             [ppmv]
!
!  NOTES:
!
!******************************************************************************
...
#     include "CMN_SIZE"          ! Size parameters
 ...
 100  FORMAT( '     - GET_DOUBLED_NH3: Reading ', a )
     ...
     &                 20,        DAT,      QUIET=.FALSE. )
      ...
      NH3_DBL(1:20)     = REAL(DAT(IIJJ(1),IIJJ(2),:),8) / 1000d0
...
!  Arguments as Output:
!  ============================================================================
!  (1 ) LAT        (REAL*8) : TES retrieval latitude
!  (2 ) LON        (REAL*8) : TES retrieval longitude
!  (3 ) BLVMR      (REAL*8) : TES retrieval BLVMR                  [ppmv]
!  (4 ) BLVMR_GC   (REAL*8) : GEOS-Chem BLVMR                      [ppmv]
!
!  NOTES:
!
!******************************************************************************
...
#     include "CMN_SIZE"
      ...
      REAL*8,             INTENT(OUT) :: LON
      REAL*8,             INTENT(OUT) :: BLVMR
      REAL*8,             INTENT(OUT) :: BLVMR_GC

      ! Local variables
      CHARACTER(LEN=255)              :: READ_FILENAME
      CHARACTER(LEN=5)                :: TMP
      REAL*8                          :: BLVMR_TMP(3)
      REAL*8                          :: BLVMR_GC_TMP(3)
      INTEGER                         :: QFLAG
      INTEGER                         :: DFLAG
...
#     include "CMN_SIZE"
      ...
      CHARACTER(LEN=255) :: FILENAME
      REAL*8  :: BLVMR_BAR(IIPAR,JJPAR),  BLVMR_GC_BAR(IIPAR,JJPAR)
      REAL*8  :: DELTA_BLVMR,           DELTA_BLVMR_GC
     ...
     &         BLVMR_GC < 0       ) CYCLE
         ...
         ! Get grid box
         IIJJ = GET_IJ( REAL(LON,4), REAL(LAT,4))
         I    = IIJJ(1)
         J    = IIJJ(2)
 ...
 100  FORMAT( '     - MAKE_TES_BLVMR: Writing ', a )
     ...
     &            IIPAR,     JJPAR,     1,     I0+1,
     &            J0+1,      1,         REAL(BLVMR_BAR,4) )

      CALL BPCH2( IU_BPCH,   MODELNAME, LONRES,    LATRES,
     &            HALFPOLAR, CENTER180, CATEGORY,  2,
     &            UNIT,      GET_TAU(), GET_TAU(), RESERVED,
     &            IIPAR,     JJPAR,     1,     I0+1,
     &            J0+1,      1,         REAL(BLVMR_GC_BAR,4) )
