...
!******************************************************************************
     ...
     &                  COST_FUNC, ADJ_FORCE, STT_ADJ
      ...
      REAL*8, ALLOCATABLE :: COST_CONTRIB(:)
...
!     &        (LATITUDE(W) .GE. -40 .AND. LATITUDE(W) .LE. 40) ) THEN  !40S-40N
            ...
            ALLOCATE( COST_CONTRIB( NLEV_RET ) )
     ...
     &                       * (LOG10( GEOS_RAW(LL) ) - LOG10( XA(LL) ))
            ...
            !DIFF_COST_COL     = MODEL_COL - MOPITT_COL
            !NEW_COST(I,J) = NEW_COST(I,J) + 0.5 * (DIFF_COST_COL ** 2) / SY
     ...
     &              (DIFF_COST(L))**2/ SY
            ...
            DIFF_ADJ(:) = 0D0
            !DO L = 1, NLEV_RET
               !DIFF_ADJ(L) = DIFF_COST_COL *  AC(L) / SY
     ...
     &                 + ADJ_F(L) * TCVV(IDTCO)/ AD(I,J,L)
            ...
            IF ( ALLOCATED(COST_CONTRIB) ) DEALLOCATE( COST_CONTRIB )
     ...
     &                             / COUNT_GRID(I,J)
               DO L = 1, LLPAR
                  IF (GC_ADJ_COUNT(I,J,L) > 0) THEN
                     ADJ_FORCE(I,J,L,IDTCO)  = ADJ_FORCE(I,J,L,IDTCO)
     &                    / GC_ADJ_COUNT(I,J,L)
                  ENDIF
               ENDDO
               NEW_COST(I,J)       = NEW_COST(I,J) / COUNT(I,J)
     ...
     &              MOP_CO_CHI_SQ(I,J,6)/MOP_BIAS_COUNT(I,J,6)
            ...
            WRITE(203,110) (GET_YMID(J))
            WRITE(204,110) (GET_XMID(I))
            !ENDDO
 110        FORMAT(F18.6,1X)
         ENDDO
      ENDDO
