...
#include "CMN_SIZE" ! size parameters
    ...
    CALL EXPAND_DATE( FILENAME_OSIRIS, YYYYMMDD, HHMMSS )
    FILENAME = TRIM( DIR_OSIRIS ) // FILENAME_OSIRIS
             ...
             IF( ( OSIRIS_DATA(N,L) > 0d0    ) .AND. &
                ...
                IF( OSIRIS_DATA(N,L) < 100d0 * OSIRIS_ERR(N,L) ) THEN 
                   ...
                   IF ( ( COST_CONT(L) > 0d0) .AND. &
                      ...
                      NEW_COST(I,J,L)  = NEW_COST(I,J,L) + COST_CONT(L)
               ...
               IF ( SOBS_COUNT(I,J) > 0d0 ) THEN
                  DO L=1,LLPAR
                     IF ( ( GET_PCENTER(I,J,L) < 300d0 )  .AND. &
                        ...
                        COST_FUNC = COST_FUNC + NEW_COST(I,J,L)/GC_ADJ_COUNT(I,J,L)
                     ENDIF
